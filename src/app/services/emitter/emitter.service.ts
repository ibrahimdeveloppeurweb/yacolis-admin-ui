...
export class EmitterService {
  ...
  emit(data: { action: string; payload?: any; }): void {
  ...
  on(event: any, callback: (arg0: any) => void): void {
  ...
  camelize(str: string): string {
    ...
    const capital = arr.map((item: string, index: any) => index ? item.charAt(0).toUpperCase() + item.slice(1).toLowerCase() : item.toLowerCase());
    return capital.join('');
  }
}
